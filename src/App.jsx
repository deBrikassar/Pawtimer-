...
const syncPush = (dogId, kind, data) => {
  sbReq("activities", {
    method: "POST",
...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Rec card ── */
  .rec-card { margin:0 24px 10px; background:linear-gradient(135deg,var(--brown) 0%,#6b5444 100%); border-radius:var(--radius); padding:18px 20px; color:white; box-shadow:0 6px 28px rgba(75,60,48,0.30); position:relative; overflow:hidden; }
  .rec-row { display:flex; gap:0; align-items:flex-start; }
  .rec-col { flex:1; display:flex; flex-direction:column; align-items:center; text-align:center; }
  .rec-divider { width:1px; background:rgba(255,255,255,0.2); align-self:stretch; margin:0 4px; }
  .rec-label { font-size:10px; text-transform:uppercase; letter-spacing:0.1em; opacity:0.75; font-weight:500; margin-bottom:6px; }
  .rec-value { font-family:'Fraunces',serif; font-size:34px; font-weight:600; line-height:1; }
  .rec-of    { font-size:18px; opacity:0.7; }
  .rec-unit  { font-size:11px; opacity:0.65; font-weight:400; margin-top:4px; }
  ...
  .cap-bar-track { height:10px; background:var(--surf-soft); border-radius:99px; overflow:hidden; display:flex; }
  .cap-bar-fill { height:100%; border-radius:0; transition:width 0.6s; flex-shrink:0; }
  ...
  .cap-bar-legend { display:flex; gap:12px; margin-top:7px; flex-wrap:wrap; }
  .cap-leg { font-size:11px; font-weight:500; }
  .cap-leg.ok   { color:var(--green-dark); }
  .cap-leg.near { color:var(--orange); }
  .cap-leg.full { color:var(--red); }
...
export default function PawTimer() {
  ...
  const recordResult = (distressLevel) => {
    ...
    setSessions(updated);
    syncPush(activeDogId, "session", session);
  ...
  const logWalk = () => {
    ...
    syncPush(activeDogId, "walk", entry);
  ...
  const logPattern = (type) => {
    ...
    syncPush(activeDogId, "pattern", entry);
    ...
    <>
      ...
      <div className="app">
        ...
        {tab === "home" && (<>
            ...
            <div className="rec-row">
              ...
              <div className="rec-col">
                ...
                <div className="rec-value">
                  {countToday}<span className="rec-of">/{PROTOCOL.sessionsPerDayMax}</span>
                </div>
          ...
          {/* Contextual tip — outside the brown card */}
          <div className="ctx green" style={{ marginTop:0 }}>
            {!sessions.length
              ...
              : !lastSess || lastSess.distressLevel === "none"
                ? (lastSess && (lastSess.actualDuration || 0) < (lastSess.plannedDuration || 0))
                  ? `Session ended early — holding at ${fmt(target)} until ${name} completes the full time.`
                  : `${name} completed the full session — stepping up +${PROTOCOL.incrementPercentDefault}% to ${fmt(target)}.`
          ...
          {/* Daily alone-time — colored segments by outcome */}
          {(() => {
            ...
            return (
              <div className="cap-bar">
                <div className="cap-bar-label">
                  ...
                  <span>{totalPlanned > 0 ? fmt(totalPlanned) : "No sessions yet"}</span>
                </div>
                <div className="cap-bar-track">
                  {totalPlanned > 0 ? (<>
                    <div className="cap-bar-fill ok"   style={{ width:`${calmPct}%` }}/>
                    <div className="cap-bar-fill near" style={{ width:`${mildPct}%` }}/>
                    <div className="cap-bar-fill full" style={{ width:`${strongPct}%` }}/>
                  </>) : <div style={{ width:"100%", height:"100%", background:"var(--surf-soft)", borderRadius:99 }}/>}
                </div>
                {totalPlanned > 0 && (
          ...
          {phase === "idle" && (<>
            ...
            <div className="howto-wrap">
              <button className="howto-toggle" onClick={() => setHowOpen(o => !o)}>
                <span>How it works</span>
                <span className="howto-arrow">{howOpen ? "▲" : "▼"}</span>
              </button>
              {howOpen && (
                <div className="ctx green" style={{ margin:"0 0 0", borderRadius:"0 0 var(--radius-sm) var(--radius-sm)", borderLeft:"none", borderTop:"1px solid var(--border)" }}>
            ...
            {countToday >= PROTOCOL.sessionsPerDayMax && (
              <div className="ctx amber">
                <strong>Heads up:</strong> You've done {countToday} sessions today. The protocol recommends max {PROTOCOL.sessionsPerDayMax} — more isn't always better. {name}'s nervous system needs time to consolidate progress.
              </div>
            )}
            {usedSec >= PROTOCOL.maxDailyAloneMinutes * 60 && (
              <div className="ctx amber">
                <strong>Heads up:</strong> You've logged {fmt(usedSec)} of alone time today. The protocol suggests around {PROTOCOL.maxDailyAloneMinutes} min/day — rest is part of the process.
              </div>
            )}

            <button className="btn-walk" onClick={logWalk}>
              <Img src="walk.png" size={28} alt="Walk"/>
              <span>Log a walk together</span>
              <span className="walk-count">Today: {todayWalks}</span>
            </button>

            {/* ── Pattern Breaking ── */}
            <div className="howto-wrap" style={{ margin:"0 24px 16px" }}>
              <button className="howto-toggle" onClick={() => setPatOpen(o => !o)}
                style={{ borderLeftColor:"var(--amber)" }}>
                <span>Pattern Breaking <span className="pat-badge" style={{ marginLeft:8 }}>Daily habit</span></span>
                <span className="howto-arrow">{patOpen ? "▲" : "▼"}</span>
              </button>
              {patOpen && (
                <div style={{ background:"var(--surf)", padding:"0 0 12px", borderTop:"1px solid var(--border)" }}>
                  <div className={`pat-reminder ${behind ? "warn" : ""}`} style={{ margin:"10px 12px 8px", borderRadius:8 }}>
                    {patReminderText}
                  </div>
                  <div className="pat-btns" style={{ padding:"0 12px" }}>
                    {PATTERN_TYPES.map(pt => (
                      <button key={pt.type} className="btn-pat" onClick={() => logPattern(pt.type)}>
                        <Img src={pt.icon} size={32} alt={pt.label}/>
          ...
          <div className="section">
            ...
            <div className="streak-card">
              ...
              <div className="streak-lbl">
                Calm session streak&nbsp;
                <img src={ICONS.streak} width={20} height={20} alt="streak"
                  style={{ verticalAlign:"middle", borderRadius:4, mixBlendMode:"multiply" }}/>
              </div>
            </div>
