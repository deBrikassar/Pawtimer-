...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .clear-btn { background:none; border:none; color:var(--text-muted); font-size:12px; cursor:pointer; text-decoration:underline; padding:4px; font-family:'Plus Jakarta Sans',sans-serif; }
      ...
      <div className="app">
        ...
        {tab === "home" && (<>
          ...
          {phase === "idle" && (<>
            ...
            <button className="btn-start-hero" onClick={startSession}>
            ...
            <p className="status-msg">
              {!sessions.length
                ? `Starting below ${name}'s comfort threshold — small steps.`
                : !lastSess || lastSess.distressLevel === "none"
                  ? (lastSess && (lastSess.actualDuration||0) < (lastSess.plannedDuration||0))
                    ? `Holding at ${fmt(target)} until ${name} completes the full session.`
                    : `${name} completed the full session — stepping up to ${fmt(target)}.`
                  : lastSess.distressLevel === "mild"
                    ? `Holding at ${fmt(target)} until ${name} is reliably calm.`
                    : `Rolled back to ${fmt(target)} — steady progress matters most.`}
            </p>
          ...
          {phase === "running" && (
            <div className="focus-screen">
              ...
              <div className="focus-ring-wrap">
                <svg className="focus-ring-svg" width={220} height={220} viewBox="0 0 220 220">
                ...
                <div className="focus-ring-inner">
                  <div className="focus-time">{fmt(Math.max(target - elapsed, 0))}</div>
        ...
        {tab === "progress" && (
          <div className="section">
            ...
            <div className="stats-row">
              ...
              <div className="stat-card">
                <div className="stat-val">{totalCount>0?Math.round((noneCount/totalCount)*100):0}<span style={{fontSize:14,fontWeight:400}}>%</span></div>
            ...
            {chartData.length > 1 ? (
              ...
              <div className="empty-state"><div className="big"><PawIcon size={48}/></div><p>Complete 2+ sessions to see {name}'s progress chart.</p></div>
            )}
          </div>
        )}
        ...
        {tab === "tips" && (
          <div className="section">
            ...
            <div className="share-card">
              ...
              {PATTERN_TYPES.map(pt => (
                <div key={pt.type} className="pat-edit-row">
                  ...
                  <button className="pat-edit-btn" onClick={() => setEditingPat(pt.type)}>✎</button>
                  {patLabels[pt.type] && (
                    <button className="pat-edit-reset" onClick={() => setPatLabels(prev => { const n={...prev}; delete n[pt.type]; return n; })}>↩</button>
                  )}
                </div>
              ))}
            </div>
              ...
              {!protoWarnAck ? (
                <div className="proto-warn-banner">
                  ...
                  <button onClick={() => setProtoWarnAck(true)}
                    style={{ marginTop:10, padding:"8px 16px", borderRadius:99, border:"1.5px solid var(--amber)", background:"transparent", color:"var(--amber)", fontFamily:"'Plus Jakarta Sans',sans-serif", fontSize:13, fontWeight:600, cursor:"pointer" }}>
                    I understand — let me edit
                  </button>
                </div>
              ) : (
                <div>
                  <div style={{ fontSize:11, color:"var(--amber)", fontWeight:600, marginBottom:10, textTransform:"uppercase", letterSpacing:"0.08em" }}>⚠️ Advanced — edit with caution</div>
                  {[
                  ...
                  <button onClick={() => { setProtoOverride({}); setProtoWarnAck(false); }}
                    style={{ marginTop:12, fontSize:12, color:"var(--text-muted)", background:"none", border:"none", cursor:"pointer", textDecoration:"underline" }}>
                    Reset to defaults
                  </button>
                </div>
              )}
            ...
            <button className="settings-btn" onClick={() => setScreen("select")} style={{ display:"flex", alignItems:"center", gap:8 }}>
              <PawIcon size={18}/> Switch to another dog
            </button>
