...
/* ==========================================================================
   КНОПКА СЕССИИ: ЭФФЕКТ ВОЛНЫ И ПЛАВНЫЙ ТАЙМЕР
   ========================================================================== */

/* 1. Делаем саму кнопку контейнером для волны */
.session-control-button, 
[class*="session-control"], 
button[class*="Train"] {
  position: relative;
  overflow: hidden; /* Чтобы волна не вылетала за края кнопки */
  transition: transform 0.4s cubic-bezier(0.34, 1.56, 0.64, 1), background-color 0.3s ease;
  background: var(--session-control-idle-bg) !important;
  box-shadow: var(--session-control-idle-shadow) !important;
  backdrop-filter: blur(20px);
  -webkit-backdrop-filter: blur(20px);
}

/* 2. Логика эффекта водяной волны при клике */
.session-control-button::after,
[class*="session-control"]::after {
  content: "";
  position: absolute;
  top: 50%;
  left: 50%;
...
/* Переход кнопки в активное состояние при нажатии */
.session-control-button:active::after,
[class*="session-control"]:active::after {
  transform: translate(-50%, -50%) scale(4); /* Волна расширяется наружу */
  opacity: 1;
  transition: transform 0s, opacity 0s; /* Мгновенный сброс при отпускании */
}

/* 3. Делаем круговой таймер идеально плавным */
svg circle, 
circle[class*="progress"], 
[class*="track-stroke"] {
  /* Переводим анимацию на видеокарту для устранения рывков */
  will-change: stroke-dashoffset; 
  /* Добавляем CSS-переход. Теперь при обновлении секунд из JS линия будет перетекать плавно, а не прыгать */
  transition: stroke-dashoffset 1s linear !important; 
}

/* Небольшая пульсация кнопки, когда таймер запущен */
.session-is-running {
  animation: pulse-glass 3s infinite ease-in-out;
  box-shadow: var(--session-control-running-shadow) !important;
}

@keyframes pulse-glass {
  0% { transform: scale(1); }
  50% { transform: scale(1.02); background: rgba(255, 255, 255, 0.55); }
