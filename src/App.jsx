...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .prog-section { padding:0 24px 8px; }
  .prog-header  { display:flex; justify-content:space-between; align-items:baseline; margin-bottom:8px; }
  .prog-goal-label { font-size:13px; color:var(--text-muted); font-weight:400; }
  .prog-pct     { font-family:'Fraunces',serif; font-size:18px; color:var(--green-dark); font-weight:600; }
  .prog-track   { height:8px; background:var(--surf-soft); border-radius:99px; overflow:hidden; box-shadow:inset 0 1px 3px rgba(75,60,48,0.08); }
  ...
  .prog-meta    { display:flex; justify-content:space-between; margin-top:6px; font-size:11px; color:var(--text-muted); }

  /* ── Hero start button ── */
  .btn-start-hero { display:flex; flex-direction:column; align-items:center; justify-content:center; width:calc(100% - 48px); margin:20px 24px 0; padding:26px 20px; background:var(--green); color:var(--brown); border:none; border-radius:24px; cursor:pointer; box-shadow:0 8px 32px rgba(168,213,186,0.55); transition:transform 0.15s,box-shadow 0.15s; gap:6px; }
  .btn-start-hero:hover { transform:translateY(-2px); box-shadow:0 12px 40px rgba(168,213,186,0.65); }
  .btn-start-hero:active { transform:translateY(0); }
  .btn-start-label { font-family:'DM Sans',sans-serif; font-size:18px; font-weight:600; letter-spacing:0.08em; text-transform:uppercase; }
  .btn-start-dur   { font-family:'Fraunces',serif; font-size:28px; font-weight:600; line-height:1; }
  ...
  .info-sep  { width:1px; height:36px; background:var(--border); }
  .info-label { font-size:10px; text-transform:uppercase; letter-spacing:0.09em; color:var(--text-muted); font-weight:500; }
  .info-val   { font-family:'Fraunces',serif; font-size:22px; color:var(--brown); font-weight:600; line-height:1; }
  .info-of    { font-size:16px; color:var(--text-muted); font-weight:400; }
  ...
  /* ── Tool rows ── */
  .tool-row { display:flex; align-items:center; justify-content:space-between; margin:0 24px 8px; background:var(--surf); border-radius:var(--radius-sm); padding:13px 16px; box-shadow:0 2px 8px rgba(75,60,48,0.06); cursor:pointer; transition:background 0.15s; }
  ...
  .alone-card   { margin:0 24px 4px; background:var(--surf); border-radius:var(--radius-sm); padding:13px 16px; box-shadow:0 2px 8px rgba(75,60,48,0.06); }
  .alone-empty  { font-size:13px; color:var(--text-muted); }
  .alone-track  { height:10px; background:var(--surf-soft); border-radius:99px; overflow:hidden; display:flex; }
  ...
  /* ── Notification toggle ── */
  .notif-time-input { border:1.5px solid var(--border); border-radius:8px; padding:5px 8px; font-family:'DM Sans',sans-serif; font-size:13px; color:var(--brown); background:var(--surf-soft); outline:none; }
  .notif-toggle { padding:6px 14px; border-radius:99px; border:1.5px solid var(--border); background:var(--surf-soft); color:var(--text-muted); font-family:'DM Sans',sans-serif; font-size:13px; font-weight:500; cursor:pointer; transition:all 0.2s; }
  ...
  .howto-toggle { width:100%; display:flex; justify-content:space-between; align-items:center; padding:11px 16px; background:var(--surf); border:none; font-family:'DM Sans',sans-serif; font-size:13px; font-weight:500; color:var(--brown); cursor:pointer; border-left:3px solid var(--green-dark); }
  ...
  .pat-edit-input { flex:1; padding:7px 10px; border:1.5px solid var(--green-dark); border-radius:var(--radius-sm); font-family:'DM Sans',sans-serif; font-size:14px; color:var(--brown); background:var(--surf-soft); outline:none; }
            ...
            <div className="header-right">
              {SB_URL && (
                <div className={`sync-dot sync-${syncStatus}`} title={
                  syncStatus === "ok" ? "Synced" : syncStatus === "syncing" ? "Syncing…" : syncStatus === "err" ? "Sync error — check Supabase setup" : ""
                }/>
              )}
        ...
        {tab === "home" && (<>
          ...
          <div className="prog-section">
            <div className="prog-header">
              <span className="prog-goal-label">Goal: {fmt(goalSec)}</span>
              <span className="prog-pct">{Math.round(goalPct)}%</span>
            </div>
          ...
          {phase === "idle" && (<>
            ...
            <button className="btn-start-hero" onClick={startSession}>
              ...
              <span className="btn-start-dur">▶ {fmt(target)}</span>
            </button>
            ...
            <div className="info-row">
              <div className="info-col">
                ...
                <span className="info-val">{fmt(target)}</span>
              </div>
              <div className="info-sep"/>
              <div className="info-col">
                <span className="info-label">Sessions today</span>
                <span className="info-val">{countToday}<span className="info-of">/{PROTOCOL.sessionsPerDayMax}</span></span>
              </div>
            </div>

            {/* Advisory warnings */}
            {countToday >= PROTOCOL.sessionsPerDayMax && (
              <p className="status-msg" style={{ color:"var(--amber)" }}>
                ⚠️ {countToday} sessions today — the protocol recommends max {PROTOCOL.sessionsPerDayMax}.
              </p>
            )}

            {/* 5. Daily alone-time */}
            <div className="tool-section-title">Daily alone-time</div>
            {(() => {
              ...
              return (
                <div className="alone-card">
                  {totalPlanned === 0
                    ? <span className="alone-empty">No sessions yet</span>
                    : (<>
                        ...
                        <div className="alone-legend">
                          ...
                          <span style={{marginLeft:"auto",fontSize:11,color:"var(--text-muted)"}}>{fmt(totalPlanned)} total</span>
                        </div>
                      </>)
                  }
                </div>
            ...
            {howOpen && (
              <div className="tool-expand">
                ...
                <div className="proto-section">
                  ...
                  <div className="proto-row">📅 Max {PROTOCOL.sessionsPerDayMax} sessions · max {PROTOCOL.maxDailyAloneMinutes} min alone/day</div>
                  <div className="proto-row">🔁 Pattern breaks: {PROTOCOL.desensitizationBlocksPerDayRecommendedMin}–{PROTOCOL.desensitizationBlocksPerDayRecommendedMax}/day · ≥ number of walks</div>
                  <div className="proto-row">😴 Rest days: {PROTOCOL.restDaysPerWeekRecommended}/week recommended</div>
                </div>
              </div>
            )}
          ...
          <div className="section">
            ...
            <div className="streak-card">
              ...
              <div className="streak-lbl">
                <span style={{ fontSize:26, verticalAlign:"middle", marginRight:6 }}>🔥</span>
                Calm session streak
              </div>
            </div>
