...
export default function PawTimer() {
  ...
  const progressMetricExplainers = useMemo(() => ({
    ...
    nextTarget: {
      title: "Next target",
      body: "The recommended duration for the next training session.",
    },
    risk: {
      title: "Risk",
      body: "The risk of stress during the recommended session.",
    },
  }), [appData.name]);
  ...
  const openMetricHelp = (metricKey) => {
    ...
    setMetricHelpSequenceIndex(null);
  };
  const closeMetricHelp = () => {
    setMetricHelp(null);
    setMetricHelpSequenceIndex(null);
  };
  const advanceMetricHelp = () => {
    if (metricHelpSequenceIndex == null) {
      closeMetricHelp();
      return;
    }
    const nextIndex = metricHelpSequenceIndex + 1;
    if (nextIndex >= PROGRESS_METRIC_TOUR_ORDER.length) {
      closeMetricHelp();
      return;
    }
    setMetricHelp(PROGRESS_METRIC_TOUR_ORDER[nextIndex]);
    setMetricHelpSequenceIndex(nextIndex);
  };
