    ...
    <div className="chart-wrap">
      ...
      <ResponsiveContainer width="100%" height={200}>
        <LineChart data={chartData} margin={{top:5,right:24,left:-14,bottom:5}}>
          ...
          <XAxis dataKey="session" tick={{fontSize:"var(--text-sm)",fill:"var(--text-muted)",fontWeight:400,fontFamily:"var(--font-main)"}} tickLine={false} axisLine={false}/>
          <YAxis tick={{fontSize:"var(--text-sm)",fill:"var(--text-muted)",fontWeight:400,fontFamily:"var(--font-main)"}} tickLine={false} axisLine={false}/>
          <Tooltip contentStyle={{background:"var(--brown)",border:"none",borderRadius:10,color:"white",fontSize:"var(--text-sm)",fontWeight:400,fontFamily:"var(--font-main)"}} labelStyle={{color:"var(--green-light)",fontSize:"var(--text-sm)",fontWeight:400,fontFamily:"var(--font-main)"}} formatter={(v,n,p)=>[`${v}m — ${distressLabel(p.payload.distressLevel)}`,"Duration"]}/>
          <ReferenceLine y={goalSec/60} stroke="var(--green-dark)" strokeDasharray="4 4" label={{value:"Goal",position:"right",fontSize:"var(--text-sm)",fill:"var(--green-dark)",fontWeight:400,fontFamily:"var(--font-main)"}}/>
