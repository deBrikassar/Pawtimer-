...
export function selectAppData({ dogs, activeDogId, sessions, walks, patterns, feedings, target, protoOverride }) {
  ...
  const patReminderText = (() => {
    ...
    if (pattern.behind) {
      ...
      return `⚠️ You've logged ${pattern.todayWalks} walk${pattern.todayWalks !== 1 ? "s" : ""} but only ${pattern.todayPat} pattern break${pattern.todayPat !== 1 ? "s" : ""}. Do ${deficit} more — with ~${pattern.normalizedLeaves} daily departures we add a ${pattern.walkBuffer} extra-cue safety buffer so pattern breaks clearly outnumber full departures.`;
    }
